...
# 3. Construct covariate summary scores
...
cat(sprintf("[%s] Constructing covariate summary scores...\n", Sys.time()))
diet_covariate <- process_covariate(diet_data, "diet")
medication_covariate <- process_covariate(medication_data, "medication")
urbanization_covariate <- process_covariate(urbanization_data, "urbanization")
general_metadata_covariate <- process_covariate(general_metadata_data, "general_metadata")
...
combined_metadata <- metadata %>%
  ...
  inner_join(urbanization_covariate, by = "sample_id") %>% inner_join(general_metadata_covariate, by = "sample_id") %>%
...
analyze_ethnicity_no_cov_filter <- function(target_eth, data) {
  ...
  for (i in seq_along(vsv_names)) {
    ...
    current_data <- data %>%
      ...
      drop_na(expression, ethnicity_binary, Age, Sex, BMI, residency,
              diet, medication, urbanization, general_metadata,
              species_abundance, species_coverage)  # no coverage cutoff
    
    if (nrow(current_data) < 10) next
    
    # include coverage as a covariate without applying a cutoff
    model_formula <- as.formula(
      "expression ~ ethnicity_binary + species_coverage + species_abundance + Age + Sex + BMI + residency +
       diet + medication + urbanization + general_metadata"
    )
...
if (nrow(sig_ethnicity_pairs) > 0) {
  cat(sprintf("\nRunning 10-fold CV for %d significant ethnicity-vSV pairs (no coverage cutoff)...\n", nrow(sig_ethnicity_pairs)))
  cv_eth_list <- list()
  ...
  for (i in 1:nrow(sig_ethnicity_pairs)) {
    ...
    res_cv <- run_cv_with_varimp_ethnicity_no_cov_filter(eth, vsv, combined_metadata, vsv_scaled)
  ...
  openxlsx::write.xlsx(cv_eth_df, "results/cv_10fold_ethnicity_sig_svs_no_filter.xlsx", rowNames = FALSE)
  cat("Saved: results/cv_10fold_ethnicity_sig_svs_no_filter.xlsx\n")
}
